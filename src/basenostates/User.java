...
public class User {
  ...
  Logger logger = LoggerFactory.getLogger("basenostates.User");
  ...
  public User(String name, String credential) {
    ...
    this.group = null;
    logger.warn("User" + this.name + "  with credential " + this.credential + " has null group assignation for now");
  }
  ...
  public void setUserGroup(UserGroup group) {
    this.group = group;
    logger.info("User" + this.name + "  with credential " + this.credential + " is assigned to" + group.getName() + "userGroup");
  }
